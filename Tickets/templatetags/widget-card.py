...
@register.simple_tag
def widget_card(title, count, color='primary', url=None, icon="", extra_class='', size='sm', btn_icon="arrow-right",
                btn_text="Read More", title_color=None, btn_color=None):
    ...
    # اضافه کردن کلاس‌های ثابت برای ارتفاع یکسان
    ...
    if count is not None:
        count_html = format_html(
            '<div class="h2 fw-bold text-dark text-center mb-3">{}</div>',
            esc(count)
        )
